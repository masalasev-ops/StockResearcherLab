...
| 5.13 | The readiness check. `digest.readiness_check_et` at 15:30, run as a Worker command because there is no UI until phase 9, reporting which link would answer and what model is loaded. **It writes nothing and decides nothing**, which is what keeps it from becoming a second gate. **Its job is to make the model resident rather than to report that it is not** [amended 2026-08-25, operator direction]. The cold load measures 41 to 52 seconds against `digest.health_timeout_ms` of 5,000, so the local link is unhealthy on exactly the nights nobody has been using the machine, and the chain then falls through to the paid link for a reason that has nothing to do with the model. **The timeout is not widened**, because one wide enough to absorb a cold load is wide enough to hide one |
