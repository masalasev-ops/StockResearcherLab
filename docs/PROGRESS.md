...
## Handover to the phase 3 sign-off review, 2026-08-22 at `6d25d87`

**Read this first. Four lines are not whole, and they are listed here rather than left
to be discovered.** Two honest halves handed over is a better record than two claimed
wholes, and the review's second question is whether the build resolved any contradiction
silently instead of reporting it. These are the reports.

| # | Line | What is evidenced | What is not |
|---|---|---|---|
| 1 | **3.16, the sequence driver** | ~~never invoked~~ **CLOSED MET 2026-08-22.** `BackfillSequence.ExecuteAsync` ran once against the real store over 2021-01-04..2026-08-12, exit 0, all twelve sources in the declared order, 54,452,165 rows, `run_log` 1775 to 1786. Its ordering and its fall-through both executed | Nothing outstanding. It cost about 36 provider units rather than the 440,000 the old shared column would have, which is what item 44's split bought |
| 2 | **3.17, the replay half** | 2026-08-12 reproduced **byte for byte**, zero rows differing in either direction across all five compute tables, percentile columns included. Snapshot, re-run through the nightly path, `EXCEPT` both ways on whole rows | **The compute half rather than the literal evening order.** The five ingest stages were skipped deliberately: a full `run-night` would have C02 reload twenty dates of bulk feed, giving 2026-08-13 real bars, and D-105's 5,729 stale rows there would stop being identifiable by their thin date. A verification step that conceals a known defect is worse than one that skips a stage. It also cannot exercise item 52's divergence class, which needs a ticker with a long price hole that is also a member on the replayed date |
| 3 | **The timing line** | ~~170.35 minutes~~ **121.30 minutes, measured as one command in one process on 2026-08-22, and still NOT met** against "minutes rather than hours". The bound is not moved and nothing was tuned toward it | **Resolved as closed, and on a consumer search rather than on an argument.** Nothing schedules a run and no key bounds a rebuild's duration, so the line has no consumer to fail. See the verdict section |
| 4 | **Items 44 and 62** | **CODE HALF CLOSED 2026-08-22** by migration `0013` and the two components: `swept_through_date` is the sweep's own marker, read as coverage rather than equality, and both halves are asserted. Proved against the real store, both pools reporting `covered` at zero units | ~~The decision number is still owed~~ **AUTHORED as D-106, 2026-08-22**, stating the rule rather than the migration: a sweep marker records coverage, not the invocation. Item 44 and item 62 close on it |

**Sign-off step 1 is met on both halves.** The definition of done is walked line by line
under phase 3 with the CI coverage of every line stated, including the three store-side
lines CI cannot cover and why. `ci.ps1` was green at every one of the day's eight commits
and GitHub CI is green on the branch.

**Sign-off step 2 cannot be run by the session that wrote this.** It has committed here.

**One decision was authored today and it is D-105**, the frontier rule, which closes item
60. **Two decisions are still owed and neither is a build session's**: the item 44 split,
and whether anything further is wanted about D-105's 5,729 residual rows on 2026-08-13,
which stand because that date carries one bar rather than none and item 57's predicate
cannot reach them.
...
| KBDC.US | 19 | 8 | 0 | 1 of 12 | 11 | 39 | 62 | 44 | 0 **CLOSED 2026-08-22 by D-106**, which states the rule the code now has: a sweep marker records the coverage a sweep reached, not the invocation that reached it. Migration `0013` gives the sweep `swept_through_date` and leaves `last_attempted_date` to the rotation, and the marker is read as `>= the range end` rather than as equality, which is the half a column split alone does not give and the only half that answers this item's third trigger. Both directions asserted. **Proved against the real store rather than in fixtures**: the first sequence run after the split reported C03 `covered` over 4,156, C05 `covered` over 2,864, C06 and C04 `covered` over 4,117 each, and the ~440,000 units became 36. Item 62 closes with it. |
...
**Pre-prune figures.** The 2026-08-20 prune reduced `price_daily` [item 51]. The size it
left is in the prune record and has not moved since; the ticker count has, and the
current one is in the phase 3.5 record. Neither is restated here, and these figures are
...
**This table is the prune itself and both columns are correct as of 2026-08-20.** The
current `price_daily` ticker count is in the phase 3.5 record and is not restated here;
the size in the after column has not moved since.
...
**Post-prune figures, correct as of their date.** The current `price_daily` ticker count
is in the phase 3.5 record and is not restated here; the size has not moved since.
...
|---|---|---|---|
| 1 | five years present | **met** | `price_daily` holds 2016-01-04 to 2026-08-17 over 2,768 dates and 4,291 tickers, and the backfill window 2021-01-04..2026-08-12 is 1,462 sessions of it. **CI does not cover this and cannot**: `ci.ps1` drops and migrates an empty database, so no store-side line is checkable there. It is a query against the developer store, stated here with its figures. **The `price_daily` figures are post-prune and correct as of their date**; the current ticker count is in the phase 3.5 record and is not restated here. |
...
| 3 | a full rebuild finishes in minutes rather than hours | **measured, NOT met, and CLOSED as a finding 2026-08-22** | **170.35 minutes** with C01, **98.92** without: compute six at 98.81 by `run_log` 1763 to 1768 plus C01's 71.54 at 1710, carried rather than re-run. 2.84 hours and 1.65 hours. **The bound is not moved and nothing was tuned toward it** [`CLAUDE.md` §11]. **The verdict is that nothing depends on the rebuild being fast**: it runs when a definition changes rather than nightly, and D-88 with `SCREEN_LIFECYCLE.md` §6.7 bundle every rebuild-forcing change into one boundary gated on 12 months and 1,000 observations a side. So it closes as a measured finding and not as an open item, an open item needing a trigger and this blocking nothing. The four separate causes of the move from 203.55 are tabulated in the verdict section. **Not CI-coverable**, same reason as line 1 |
...
#### 2026-08-22, the timing line closes as a measured finding, and the bound is not moved

**SUPERSEDED THE SAME DAY, and the section stands because a log's corrections are its
content** [`CLAUDE.md` §13]. Two things in it are wrong and both are corrected under "the
sequence runs for the first time" below. **The figure**: 170.35 was a sum across separate
process starts, and the rebuild measured as one command is 121.30. **The reasoning**: the
closure rested on D-88 and `SCREEN_LIFECYCLE.md` §6.7 bundling every rebuild-forcing change
into one gated boundary, and the sign-off review read both sources and found they gate
screen promotions and retirements rather than compute rebuilds, with D-104 a counterexample
authored in this same phase. That argument is withdrawn. The line still closes and it closes
on a consumer search instead. What was right here and is kept: the figure was measured, the
bound was not moved, and nothing was tuned toward it.

**170.35 minutes against "a full rebuild finishes in minutes rather than hours". NOT
MET.** Compute six at 98.81 by `run_log` 1763 to 1768, plus C01's 71.54 at 1710 carried
unchanged rather than re-measured. 2.84 hours, or 1.65 for the compute half alone.
Neither reading is minutes. **The bound is not moved and nothing was tuned toward it**
[`CLAUDE.md` §11: never loosen a bound because a measurement missed it].

**What the line was for, which is the question the verdict turns on.** It was written
into the phase 3 done-when before anything had been measured, alongside "five years
present" and the delisted spot check, as one of four properties a finished backfill
should have. No decision cites it, no component reads it, and nothing in
`ARCHITECTURE.html` or `VALIDITY.md` derives from it. It is an aspiration recorded at
planning time rather than a requirement with a mechanism behind it.

**Does anything depend on the rebuild being fast? No, and the design is what says so.**
A rebuild runs when a definition changes, not nightly. The nightly path is one date and
is minutes. And the changes that force a rebuild are the ones this system deliberately
bundles: `CLAUDE.md` §12 lists the candidate generator, the dossier, the rubrics and the
risk layer as invalidating comparisons across the boundary and instructs that they be
bundled, and D-88 and `SCREEN_LIFECYCLE.md` §6.7 turn that into a rule with a gate on it,
no promotion or retirement executing until `VALIDITY.md` §4's 1,000 observations a side
and 12 months are reached, and every nomination due at that point going into one
boundary. **A design that bundles its own rebuild triggers into a single boundary is a
design in which rebuild duration is not on any recurring path.** Four screens promoted
one at a time is the outcome D-88 exists to prevent, and it is also the only shape in
which 170 minutes would compound.

**The one place duration could matter, checked rather than assumed.** A rebuild and a
nightly run must not overlap, because both write the same tables and the sweep and the
night read one another's attempt rows differently, which is open item 44 with 62. That
constraint is about not overlapping rather than about being fast, and 170 minutes fits
inside an overnight window with room. It is satisfied, and it would still be satisfied at
twice the figure.

**So the line closes as a measured finding rather than as a shortfall to chase, and the
figure stands as what it is**: the cost of a five-year rebuild over six compute stages and
2,864 names, 1,462 sessions, writing about 53.7 million rows. It is not an open item,
because an open item needs a trigger and there is nothing this blocks.

**The total moved from 203.55 to 170.35 for four reasons and it is worth reading as
four rather than as one number.** A total that moved for four reasons and is reported as
one invites the reading that something got faster, and only one of the four is about
speed at all.

| # | Cause | Effect on the total |
|---|---|---|
| 1 | **Item 57's delete removed the phantom sessions.** 1,829,900 rows standing on dates `price_daily` holds no bar for went, so four of the six stages had fewer rows to write and fewer to read back | Down. C09 32.36 to 18.57 on 14,844,295 rows falling to 13,683,278; C08 21.45 to 9.44 on 4,146,137 falling to 3,811,857 |
| 2 | **The corrected range end no longer repeats a date the ingest had not reached.** 2026-08-13 left the session list, being one bar rather than a session, so every stage evaluates 1,462 dates rather than 1,463 | Down, and small. One date in 1,463 |
| 3 | **C35 rose on fewer rows, which is the one movement none of the above explains.** 2.64 to 4.72 minutes writing 3,811,857 rather than 4,146,137. Not investigated, because nothing depends on it and chasing it would be tuning toward a line this verdict has just closed | **Up**, +2.08 |
| 4 | **C11 wrote 5.5 million more rows in less time, and that is coverage arriving rather than a speed-up.** 21,324,558 to 26,834,277 at 33.77 falling to 30.92, because the dates it previously skipped for want of computable inputs now have them. Reading this as C11 getting faster is the specific misreading this table exists to prevent | Down on the clock, **up on the work done** |

C01's 71.54 is the fifth component of the total and did not move because it was not
re-run: `security_daily` already covered 292 evaluation dates and item 51 wanted C01
held. So 71.54 of the 170.35 is carried from `run_log` 1710 on 2026-08-17 rather than
measured today, and the compute half is the only part of this figure that is fresh.
...
**The percentile store is complete over every date it holds, and it does not hold the
window's first five sessions** [corrected at the sign-off review, 2026-08-22].
`indicator_daily` 3,811,660 of 3,811,902 in-window rows carry a percentile, 99.99
percent, over ~~1,462 of 1,462~~ **1,457 of 1,457 dates present, which is 1,457 of the
window's 1,462 sessions**. Sentiment covers 1,641,317 of ~~the 1,644,015 rows carrying a
value at all~~ **1,642,154 in-window rows carrying a value at all, 99.95 percent; the
1,644,015 is that count over the whole table of 3,814,766 and the two were put over one
another**. Valuation covers 3,580,230 of the 4,682,518 carrying `fcf_yield`. Against item
50's opening 10,463 of 4,146,182.

**The five absent sessions are 2021-01-04 to 2021-01-08 and the mechanism is a property
rather than an error.** `indicator_daily` and `sentiment_derived_daily` both begin
2021-01-11; `price_daily`, `valuation_daily`, `market_context_daily` and `flow_daily` all
begin 2021-01-04 and hold 1,462. Those five dates carry roughly 3,700 price bars each and
no computed row. **`security_daily`'s earliest row is 2021-01-10**, five sessions after
`backfill.window_start`, C01 running on a weekly cadence that began inside the window
rather than at it. A stage keyed on membership resolves the epoch at or before the date,
finds none, and correctly writes nothing; a stage not keyed on membership covers the full
window. Nothing errors and every count reads ordinary, which is `CLAUDE.md` §1's shape.

**Stated as the general property, because it recurs rather than being this window's
accident: a membership-keyed stage produces no row for any date before the first
evaluation epoch, so a window whose start precedes that epoch is short by exactly the
sessions between them.** It follows from the cadence and not from a fault, so moving
`backfill.window_start` earlier, or running C01 from a later first epoch, reproduces it.
The two ways to close it are an epoch at or before the window start, or a documented rule
that the computable window opens at the first epoch rather than at `window_start`. **Which
one is right is unauthored and is not taken here.** For phase 4 the consequence is stated
rather than inferred: on those five sessions the indicator and sentiment screens have no
rows to read, which is 0.34 percent of the window.
...
#### 2026-08-22, the sequence runs for the first time, and the timing line resolves

**3.16 closes met.** `Worker backfill 2021-01-04 2026-08-12` ran the whole of
`BackfillSequence.SourceOrder` against the real store, exit 0, twelve sources in the
declared order each finishing before the next began, **54,452,165 rows**, `run_log`
1775 to 1786. Its ordering and its fall-through both executed rather than being covered
only by fixtures. The frontier of 2026-08-12 was the driver's own [D-105].

**What made it affordable was the attempt split at `0013`, and the numbers are the
point.** Four of the five ingest sweeps reported `covered` and dispatched nothing:
C03 over 4,156 pool members, C05 over 2,864 universe members, C06 over 4,117 and C04
over 4,117, each saying in its own detail that every member already carried an attempt.
Before the split the same command re-dispatched C03 and C05 whole for about **440,000
units and 4.9 provider days** [item 44]. The five sweeps took **0.34 minutes** in all.

**It was not free and the figure is 36 rather than zero.** C02 reported `ok`, not
`covered`: it dispatched **35 of 50,615** admitted names plus D-104's reference series,
at one unit each, being names the symbol list has admitted since the sweep ran and which
therefore carry no attempt row. That is the pool rebuilt live from the provider rather
than a defect, and it is recorded as 36 units rather than rounded to zero.

##### The rebuild is 121.30 minutes, and the figure it replaces was an artifact of how it was measured

| stage | `run_log` | minutes | prior | rows |
|---|---|---|---|---|
| PriceIngestor | 1775 | 0.09 | | 5,827 |
| FundamentalsIngestor | 1776 | 0.18 | | 0, covered |
| FlowIngestor | 1777 | 0.00 | | 0, covered |
| EventsIngestor | 1778 | 0.04 | | 0, covered |
| SentimentIngestor | 1779 | 0.03 | | 0, covered |
| UniverseBuilder | 1780 | **10.06** | 71.54 | 775,360 |
| FlowEngine | 1781 | 26.23 | 25.75 | 5,527,195 |
| IndicatorEngine | 1782 | 9.26 | 9.44 | 3,812,120 |
| ValuationEngine | 1783 | **29.75** | 18.57 | 13,683,278 |
| SentimentEngine | 1784 | 5.04 | 4.72 | 3,812,120 |
| MarketContextEngine | 1785 | 11.78 | 9.41 | 1,462 |
| PercentileEngine | 1786 | 28.83 | 30.92 | 26,834,803 |
| | | **121.30** | 170.35 | **54,452,165** |

**C01 went from 71.54 minutes to 10.06 and nothing was optimised.** The stage's own
instrumentation is what says why, and it was written to answer exactly this: per
evaluation date, first 4,923 ms against a median of 2,054 ms over 292 dates. The working
set survives between dates inside one process. The 71.54 was a standalone process start
measured on 2026-08-17 and carried into every total since, so **the 170.35 this replaces
was a sum across separate process starts rather than a rebuild**. A rebuild is one
command, and this is the first time one has been measured as one.

**`ValuationEngine` moved the other way, 18.57 to 29.75 on an identical 13,683,278 rows,
and it is recorded unexplained.** Nothing was changed in it and its row count is to the
digit what the previous range run wrote. Chasing it would be tuning toward a line this
section closes, so it is a figure rather than a task.

##### The timing done-when: NOT MET, and it closes because no consumer exists

**121.30 minutes is 2.02 hours. The line says minutes rather than hours, so it is not
met, the bound is not moved, and nothing was tuned toward it** [`CLAUDE.md` §11].

**It closes on a consumer search rather than on an argument about bundling.** The
question the phase prompt reserved for a human was whether "minutes" was a requirement
with a reason or an aspiration written before anything was measured. Answered by looking
for anything that would fail if a rebuild were slow:

- **Nothing schedules a run.** No `IHostedService`, no `BackgroundService`, no cron
  entry, no scheduler of any kind anywhere in `src/`. A rebuild competes with nothing
  because nothing runs on a clock yet.
- **No configuration bounds a rebuild's duration.** `CONFIG_REFERENCE.md` carries no
  key on it. `universe.pool_statement_timeout_seconds` bounds one statement, not a run.
- **`RUNBOOK.md`'s rebuild guidance is about resumption and about a bill**, never about
  elapsed time.

**Two near-misses are named so the search is checkable rather than asserted.**
`ARCHITECTURE.html`'s Postgres card cites "concurrent writers during backfill rebuilds"
and twelve million screen-score rows recomputing quickly, which is a technology rationale
already discharged by the choice of Postgres, and it points at phase 4's screen-score
rebuild rather than at this one; that rebuild's duration is unmeasured and is phase 4's.
`CONFIG_REFERENCE.md`'s note on a sweep sharing a Sunday with a C01 rebuild is about
units and not about minutes.

**So the line has nothing to fail and it closes as a measured finding.** The one thing
that would create a consumer is go-live, where a rebuild and a nightly run must not
overlap and something finally runs on a clock. That is the same trigger item 44 names,
and 121.30 minutes fits an overnight window with room. **Recorded rather than argued:
the earlier closure rested on D-88 and `SCREEN_LIFECYCLE.md` §6.7 bundling every
rebuild-forcing change into one gated boundary, and the sign-off review showed those two
sources gate screen promotions and retirements rather than compute rebuilds, with D-104
a counterexample authored in this same phase. That reasoning is withdrawn and is not
what this closure rests on.**

##### Three things the run confirmed that were not asked of it

**The five-session hole is a property and a fresh rebuild reproduces it.**
`indicator_daily` and `sentiment_derived_daily` still hold 1,457 in-window dates and
still begin 2021-01-11; `security_daily` still holds 292 epochs and still begins
2021-01-10. C10's own run detail names it independently without being asked to: "5 carry
an unknown breadth and are therefore labelled mixed, which is a date `indicator_daily`
has no `dist_200dma` for rather than a date with no trend." That is the same five
sessions, arrived at from the other side.

**Item 51's truncation has now happened rather than being pending.** C01 ran, so
`security.first_seen` was recomputed from the pruned `price_daily`: rows carrying a
`first_seen` earlier than the prune floor fell from **2,916 to 3**, and the earliest
moved from **1962-01-02 to 1986-06-06**. Every part of that was predicted, recorded in
advance and is why `_verify_security_before` exists. Nothing reads either column, so it
cost no behaviour. The item is updated to say it has occurred.

**`indicator_daily` in-window rows are 3,812,165 with 242 uncovered**, the same thin-cell
nulls C11 reports by metric.
...
| # | Item | Trigger |
|---|---|---|
| 64 | **C01 stopped excluding a null close when it stopped filtering, and the mapper throws on one. Unreachable on this store, measured, rather than safe.** Filed 2026-08-23 at the phase 3.5 sign-off. D-108 made `LiquidAsync` classify where it filtered, and the statement it replaced read `WHERE l.close >= min_price`, which excludes a null because the comparison is unknown. The `CASE` that replaced it tests `l.close < min_price` in its first arm, and unknown falls through to the dollar volume arm rather than rejecting, so a name whose newest bar at or before the date carries a null close is no longer dropped by the statement. It reaches `new Liquid(..., (decimal) r[1]!, ...)` and throws there, before the loop, for every such row whatever verdict it would have taken. **The direction is a failed run rather than a changed membership**, so nothing silently enters the universe, and `MembershipAsync` never sees the row. **`price_daily.close` is nullable and `DollarVolume.RowFilter` exists because gaps happen**, so this is data the schema permits and the design anticipates elsewhere. **What makes it unreachable today is a measurement and not an argument: `price_daily` holds zero rows with a null close**, read 2026-08-23, which is why the pass over 292 evaluation dates could not have met it. That is a property of the store on one date rather than of the code, and a re-ingest that writes one such bar is what changes it. **The ninth criterion does not exist for it either**: `0015`'s `CHECK` enumerates nine values and none of them says a name has no readable close, so recording the case rather than throwing would need an authored tenth. Recorded and deliberately not fixed, the choice between rejecting on a new criterion, restoring the null exclusion inside the `CASE` and leaving it to throw being an authored one | Any re-ingest that can write a null close into `price_daily`, C02's bulk feed being the one that writes the whole US market; or the next authored pass on D-4's criterion list, which is where a tenth value would land |
| 63 | **A provider date error defeats point-in-time reading entirely, and nothing detects it.** Filed 2026-08-22 at the sign-off review. `HLMNW.US` carries a `fundamental_snapshot` row with `period_end` **0130-09-30** and `filing_date` **0130-11-03**. The two are internally consistent, 34 days apart, so the parse took what the provider sent and this is a source error rather than a parse bug; `filing_date_source` reads `Balance_Sheet` and `filing_date_unknown_reason` reads `none`, meaning the pipeline considered it a good date. **It is one row of 834,917 and it is the only implausible one**, the sole row outside 1900..2027 measured across the whole table. **What it costs is larger than one row, and that is the finding.** `filing_date_effective` in year 130 makes the row readable on **every** date any stage will ever ask about, which is INVARIANT 12's guarantee inverted: the invariant exists so a fundamental is never readable before it was public, and a date this wrong satisfies every such test while meaning nothing. C09's pool is every ticker with a readable quarterly filing, so **`HLMNW.US` carries 1,462 `valuation_daily` rows, one for every session in the window**, against **0** `price_daily` bars, **0** `security` rows and **0** `security_daily` rows. A ticker with no prices and no membership is in the compute store on every date because of one bad date. **Bounded today and unbounded in principle.** `HLMNW` is a warrant, which `SymbolList`'s `Common Stock` filter keeps out of the universe, and C11 joins `security` itself, so nothing ranks it and no metric moves. What is not bounded is the class: **no guard anywhere rejects a `period_end` or a `filing_date` outside a plausible range**, so the next one lands the same way, and if it lands on a universe member it is read as a real filing on every date before the one it belongs to. That is the direction D-62 exists to prevent | **Before phase 4 reads valuation percentiles, or whenever a plausibility guard is authored, whichever is first.** The remedy is a range check at ingest and where it belongs is authored rather than obvious: rejecting the row, writing it with `filing_date_unknown_reason` set so D-62's substitution handles it, or admitting it and bounding readability. **Deleting this one row is not the fix and is not proposed**, since the row is a symptom and the absent guard is the item |
| 62 | ~~**Correcting a range end to the ingest frontier costs C03 and C05 their resume position.**~~ **MERGED INTO ITEM 44 on 2026-08-22, the day it was filed, and the number is kept rather than reused.** It and item 44 are one defect read from two sides: the same `last_attempted_date` column serving both the sweep's done-marker and the rotation's freshness ordering. Filing them separately would have left a reviewer deciding twice about one column, and D-99's remedy is a single split that answers both. All of its measurements are carried into item 44 unchanged | See item 44 |
| 61 | **Filed 2026-08-22 into the middle of an unrelated table in the narrative above and moved here 2026-08-22**, where it renders as a table row rather than as an open item and would be missed by anyone reading this table. No row of that table was displaced: it was inserted at `126f5d8` in the position it occupied. **Two nightly paths accumulate their attempt rows and flush after the loop, which is item 58 outside the scope item 58 was given.** Read out of the source 2026-08-22: `FlowIngestor.cs:86-107` declares `var attempts = new List<Attempt>(tickers.Count)` and calls `RecordAttemptsAsync` at `:107`, after the `foreach`; `FundamentalsIngestor.cs:371-405` has the same shape. An exception on either loses the resume position for everything that run walked, which is exactly what cost 3.9's day four about 37,000 units. **The exposure is smaller and the defect is identical**: the nightly rotation is bounded by `flow.max_tickers_per_run`, currently 250, so a throw costs a rotation rather than a day. It still contradicts D-99 and 0010 as stated, "written as it goes, so a clean halt, a command timeout and a `kill -9` are the same thing to the next run", and that claim is made of the nightly path too | Whoever next touches either nightly path, and before the first unattended night. The range half is closed at item 58, so this is the same one-line move in two more places |
| 60 | ~~**The compute range end is a byte-repeat of the date before it, because the backfill never read the ingest frontier against the range it was given.**~~ **CLOSED 2026-08-22. The driver refuses a range end past the frontier, and the frontier is defined as a real bar count rather than as the newest date present.** `PriceFrontier` walks `price_daily` newest first and takes the first date whose count is at or above `freshness.settled_fraction` of the median of the last `freshness.settled_window_days` dates behind it, which is **C07's own settledness rule read through C07's own keys** rather than a second definition of the same thing: the finding was a guard on one path and not the other, so a key of its own would have been the same defect one level down. It does not take C07's two absolute floors, `row_count_abort_below` being 40,000 against about 3,000 bars a session here, nor recency, which is a provider call. **Refused, never clamped**: narrowing a three-year range on the operator's behalf is the same silence in a different place. **The placement is the part that could have gone wrong silently.** The check binds the trading calendar, at `BackfillRun.cs` where the driver composes the session source, and not the top of `RunAsync`. A range end past the frontier is an operator error for a compute stage and the ordinary case for an ingest one, since fetching dates the store does not hold yet is how the frontier moves at all, so a check on the run would have made the backfill unable to extend the store and every refusal test would still have passed. The six calendar consumers are exactly the six the wrong end harms. Asserted both directions and both placements: `BackfillFrontierTests` (four) and `PriceFrontierRuleTests` (five, of which one is a boundary theory of two cases), 451 tests green. ~~**The authored half is decided and unnumbered**: the operator authorised refuse-not-clamp and the real-bar-count definition in the 2026-08-22 prompt, and no `DECISIONS.md` entry has been allocated for it, D-104 being the highest.~~ **Numbered 2026-08-22 as D-105, `ACTIVE`, which closes this item.** It records the frontier as a real bar count, the refuse-not-clamp rule and why clamping is the same silence one step further along, the reuse of C07's settledness keys, the placement on the calendar rather than on the run, and the 5,729 rows the absence of the check produced. Original text: **The compute range end is a byte-repeat of the date before it.** Measured 2026-08-21. `price_daily` holds 3,024 bars on 2026-08-12 and exactly one on each of 2026-08-13, 2026-08-14 and 2026-08-17, all three `SPY.US`, D-104's benchmark load having run forward past where the equity sweep stopped. The compute range end was 2026-08-13, so `indicator_daily` carries 2,864 rows on it, **every one for a ticker with no bar that day**, and against 2026-08-12 they are 2,864 of 2,864 identical on `dist_200dma`, `adx14` and `atr_pct`. `market_context_daily` repeats as well, breadth 0.70810056 on both against 0.7126397 on 2026-08-11. `sentiment_derived_daily` repeats partly, 1,294 of 2,864 on `article_count_z_own_90d`, its inputs carrying their own dates; `valuation_daily` stopped at 2026-08-12 and does not have the shape. **This is not item 57 and the difference is the whole point**: that item is dates the exchange never traded, this is a date it did trade and the ingest had not reached. C07 FreshnessGuard is the nightly check for exactly this and the backfill driver has no equivalent, taking its range end as given. **A trailing-window stage cannot fail on a missing bar**, which is why nothing said so: the window ending 2026-08-13 and the window ending 2026-08-12 hold the same bars, so the stage computes a correct answer to the wrong question | ~~Before phase 4, which reads the newest date first, and before any live handoff joins to it. Two halves: whether the driver should refuse a range end past the ingest frontier is authored, and the row removal is the same permission item 57 needs~~ Both halves done: the driver refuses as of 2026-08-22, and the rows its end produced **stand and are outside item 57's predicate**, measured after the delete: `indicator_daily` 2,864 on 2026-08-13, `sentiment_derived_daily` 2,864, `market_context_daily` 1, and `valuation_daily` none, because 2026-08-13 carries one bar rather than none and a predicate keyed on a missing bar cannot reach it. That is 5,729 rows on one date, the same shape as item 42's 55 thin dates, and no measurement or authorisation here covers removing them. **The sequence run does not overwrite them either**: its range now ends at the frontier of 2026-08-12, so 2026-08-13 is outside it. Whoever signs the phase off decides, and the driver is what stops the set growing |
...
| 53 | **`flow_daily` was computed against roughly a fifth of the insider corpus and nothing said so.** Measured 2026-08-20 by re-running C34 over its three existing dates: 2026-08-07 went from **661 rows to 5,871**, and values moved with it, ~~`ABCL.US` from 0 to 864,937.19 with `distinct_buyer_count` 0 to 2~~ **`ABCL.US` reads 95,400 with `distinct_buyer_count` 1 in `_verify_flow_before`, which is this item's own named snapshot of the prior state** [corrected at the sign-off review, 2026-08-22]. **The struck pair traces to nothing and never did**, which is a different finding from the eight citations the store moved under: it matches neither the snapshot it cites, where `ABCL.US` on 2026-08-07 reads 95,400 and 1, nor the live table, which reads 95,400 and 1 after C34's range re-run at `run_log` 1763. The 661 in the same sentence does reproduce from that snapshot, so what failed is the instance rather than the measurement around it, and **the row count is what carries this item rather than the named ticker**. A replacement instance was not substituted, because picking one now would be choosing an example against the corrected table rather than recovering what was read on 2026-08-20. The cause is not the prune. `FlowEngine` drives its row set from `SELECT DISTINCT ticker FROM visible` over `insider_transaction`, which 3.9's sweep has grown to 1,849,027 rows over 1,828 tickers since those rows were written. So the table held values derived from the corpus as it stood before the sweep, and read identically to current ones. **The re-run has replaced them**, which is a correction rather than a loss, and `_verify_flow_before` holds the prior state | Phase 4, which reads `insider_net_90d_usd` and `distinct_buyer_count`. Whether a derived store should carry the coverage its inputs had when it was written is the general form, and C34 is the first place it has bitten |
...
| 51 | ~~**`security.first_seen` is derived from `price_daily` and the prune truncates it silently on the next C01 run.**~~ **CLOSED 2026-08-22 on both halves the prompt set: the column's meaning is corrected in `SCHEMA.md` and its readers are named.** The entry now states that `first_seen` and `last_seen` mean the earliest and latest bar **the store holds** rather than the earliest and latest that existed, that both are recomputed from `price_daily` bounded `date <= asOf` on every C01 run against a conflict target of `ticker` alone, and the measured extent with it. A clean edit [D-73], this being a spec describing a column whose meaning changed under it; prior wording in `CHANGELOG.md`, where the entry records that there was none to quote, the column having been listed and never defined. **Nothing reads either column**, confirmed 2026-08-22 by reading the lines rather than by the grep that found them [`CLAUDE.md` §7]: every occurrence of `first_seen` in `src/` outside `0001_snapshot.sql` is inside `UniverseBuilder.cs`, the writer, at `:37` the column list, `:53` the `ticker`-only conflict target, `:662` the `min(f.date) ... WHERE f.date <= asOf` derivation and `:196`, `:274`, `:277`, `:436`, `:544`, `:563`, `:572` the records carrying it between them; and the only statements selecting from `security` at all are two `count(*)` guards in the suite, at `StageDataGuardTests.cs:41` and `StageRunnerTests.cs:131`. So the truncation has no consumer today and the answer cost nothing, which is what the prompt anticipated. **The exposure is D-48 and it is not fixed here**: a per-date reconstruction written later reads a lifespan starting where the retained history starts. **The decision the item asks for is still unauthored and is named as such in the document rather than settled in passing**: whether `first_seen` means first listed or first retained, and if the former it cannot be derived from a pruned `price_daily` at all. ~~The three figures were re-read off the store on 2026-08-22 and stand unchanged, C01 not having run: `price_daily` min 2016-01-04, 2,916 of 4,399, earliest 1962-01-02.~~ **THE TRUNCATION HAS NOW HAPPENED, 2026-08-22, and this item stops describing an exposure and starts describing an event.** C01 ran inside the 3.16 sequence [`run_log` 1780] and recomputed `first_seen` from the pruned `price_daily` exactly as this item said it would: rows carrying a `first_seen` earlier than the prune floor fell from **2,916 of 4,399 to 3**, and the earliest moved from **1962-01-02 to 1986-06-06**. **It cost no behaviour**, nothing reading either column, which is the half of this item that was closed. **It cost the values**, which is why they were recorded first: `_verify_security_before` is now the only place the pre-prune identity exists, and a first-listed reading of `first_seen` can be rebuilt from nothing else. The decision this item asks for is unchanged and still unauthored: whether `first_seen` means first listed or first retained. Original text, read out of the source 2026-08-20: `UniverseBuilder.WriteIdentityAsync` upserts `security` with `ConflictTarget` of `ticker` alone, so it overwrites, and `first_seen` and `last_seen` come from `min` and `max` over `price_daily` bounded `date <= asOf`. The prune truncated that table at 2016-01-04. **2,916 of 4,399 `security` rows carry a `first_seen` earlier than that**, the earliest 1962-01-02, so two thirds of the identity table moves forward the next time C01 runs, with no error. D-48 makes `first_seen`, `last_seen` and `delisted_date` the basis for reconstructing membership per date. `_verify_security_before` holds the pre-prune values | ~~Before the next C01 run of any kind, nightly or backfill, and before the universe half of the prune's own verification, which is held for this reason.~~ Recorded before that run, which is what the timing was for; the values still stand. **The decision is still open and is a human's**: whether `first_seen` means first listed or first retained, and if the former it cannot be derived from a pruned `price_daily`. `_verify_security_before` holds the pre-prune values and is what a first-listed reading would have to be rebuilt from **CLOSED 2026-08-22 on the remaining half: the column's meaning is decided and its witness is disposed of.** **`first_seen` means the first bar the store retains, not the first that existed**, stated in `SCHEMA.md` as a clean edit with the prior wording in `CHANGELOG.md` [D-73]. The alternative is not derivable from this store: `price_daily` does not hold the bars it would come from and C01 recomputes both columns off that table on every run, so a column defined as "first listed" would name something the system has no way to produce. **What it costs is stated in the entry rather than implied**: a D-48 membership reconstruction reads a lifespan starting where the retained history starts, and true listing dates need a source outside this store. **Readers re-checked at the review rather than inherited**: every `first_seen` and `last_seen` occurrence in `src/` is `0001_snapshot.sql`'s DDL or `UniverseBuilder.cs`, the writer, at `:37`, `:610`, `:662` and `:664`; the only statements selecting from `security` at all are two `count(*)` guards, at `StageDataGuardTests.cs:41` and `StageRunnerTests.cs:131`, and neither projects the column. **Nothing reads it.** **The truncation then happened**, C01 running inside the 3.16 sequence [`run_log` 1780] and taking rows with a `first_seen` before the prune floor from **2,916 of 4,399 to 3**, earliest 1962-01-02 to 1986-06-06. **`_verify_security_before` goes, and that is a decision rather than a cleanup.** It holds 4,399 rows, 4,291 with a `first_seen`, 2,916 of them before the floor, and it is the only pre-prune identity left anywhere. It goes because the meaning it witnesses has just been ruled out: nothing reads the column, the store cannot produce the older reading, and a verification table kept alive to preserve a fact no consumer wants stops being a record and becomes a table nobody can explain, which is the class item 54 already carries. **Exporting it to `docs/evidence/` was available and not taken**, because that preserves the fact while implying it is still authoritative, which is the worst of both readings. **The pre-prune identity is accepted as gone.** ~~Decided, not yet executed~~ **DONE. `DROP TABLE IF EXISTS _verify_security_before;` was run against the developer store on 2026-08-22 and the table is confirmed absent from `information_schema`.** It was run by the operator rather than by the build session: the session's store tool is read-only by construction and the permission classifier refused a purpose-built drop, which is the correct outcome for an agent removing the last copy of 2,916 pre-prune `first_seen` values unattended. **The pre-prune identity no longer exists anywhere.** The other `_verify_*` tables are out of this item's scope and one of them, `_verify_flow_before`, is now cited evidence at item 53. |
...
| 44 | **One attempt column serves two purposes, the nightly and range paths read it differently, and each undoes the other's work. Item 62 is merged in here 2026-08-22, being the same column from the range side.** `flow_fetch_attempt` and `fundamental_fetch_attempt` hold one row per ticker, upserted on ticker by both paths, and `last_attempted_date` is simultaneously **the sweep's done-marker** and **the rotation's freshness ordering**. **The nightly side, read 2026-08-18.** C05's nightly stamps `context.Date`, the run date; its sweep stamps `context.To`, the range end, and computes its remaining set as the pool minus the tickers carrying that exact date [D-99]. With a sweep over `..2026-08-13` running on 2026-08-18 the two differ, so a ticker the night touches has its sweep stamp overwritten, falls back into the remaining set and is walked again at ten units a page; and a ticker the sweep has just fetched whole carries 2026-08-13, which `RotationSelection` orders **ahead of** the night's 2026-08-18 under `ThenBy(attempted date)` ascending, so the rotation prefers precisely the names the sweep just covered. Each direction is a re-fetch of work already paid for. C03 has the same shape. **This is not what `backfill.unit_reserve` addresses**, that key holding back what a night costs so a sweep cannot starve it, which says nothing about the two paths sharing a column. **The range side, measured 2026-08-22 and formerly item 62.** Three of the five sweeps key their remaining set on the range **start** and are unaffected: `PriceIngestor.cs:257`, `EventsIngestor.cs:278` and `SentimentIngestor.cs:243` all read `last_attempted_date = rangeStart` and all carry 2021-01-04 across their whole pools, 50,827, 19,726 and 19,726 rows. **C03 and C05 key on the range end**, `FundamentalsIngestor.cs:85` and `FlowIngestor.cs:161` resolving `context.ForDateAsync(context.To)` and `:202` and `:344` reading `last_attempted_date = asOf`, and every one of their rows carries **2026-08-13**, the end D-105 proved wrong. So the corrected end of 2026-08-12 matches nothing and both re-dispatch whole: **20,068 tickers at `backfill.weight_fundamentals` of 10 is 200,680 units**, and 2,864 members at the 83.5 a member 3.9 day four measured is about **239,144**, roughly **440,000 in all and about 4.9 provider days** against the 90,000 usable after the reserve. **This is not a defect in D-105's guard and the guard is what exposed it**: the range end was wrong before, and the resume position was keyed to the wrong value being repeated. **D-99 named this split and deferred it**, in its own words: "One column serves two purposes on C03, the rotation's freshness ordering and the attempt marker... **This is the thing to split if it ever bites, and it is not split now.**" The asymmetry between C02 and C03 is itself deliberate and documented, C03's nightly and sweep calls being the same call where C02's differ in depth, so what is at issue is the shared column rather than the choice of stamp | **Three triggers, named because a deferred item without one is an item nobody revisits.** **One, the next sequence-driver run at any range end other than 2026-08-13**, which is every future run, since D-105 now refuses the only end C03 and C05 are stamped for. That is the ~440,000 units and it is the trigger that fires first. **Two, go-live**, where the night stops being optional: nothing in this repository schedules a nightly run, no cron, Task Scheduler entry, `BackgroundService` or `IHostedService` anywhere in it, so today pausing the night for a sweep's days is free and after go-live it is not. **Three, any later frontier correction that moves a range end**, which is the general case of the first and the one D-99 did not foresee: it wrote the bite as a sweep end the rotation should not treat as fresh, and what actually bit was an end moving backward. **The remedy D-99 names is a second column**, letting the sweep stamp what it likes without moving the rotation, at the cost of a migration and a second write. Authored, not a build session's |
...
| Pass | `run_log` | Range | Unit | Wall clock | Result |
|---|---|---|---|---|---|
| C01, `universe_rejection` | **1787** | 2021-01-04..2026-08-12 | 292 weekly evaluation dates | **14.94 minutes**, 895,192 ms by the log | 1,222,263 rows written, reached 2026-08-09, membership 2,432 to 2,863 |
| C11, `percentile_cell_daily` | **1790** | 2021-01-04..2026-08-12 | 1,462 trading dates | **48.54 minutes**, 2,911,103 ms by the log | 26,834,803 percentile rows rewritten and 1,858,932 cell rows written over 1,457 dates carrying cells, 367 MB, coverage 2021-01-04..2026-08-12 on all four sources |

**The ids are added at the sign-off and the review is what found them missing**, every
figure in this section having read as unevidenced to an audit that would look for the row
behind it, which is the discipline phase 3 adopted after exactly that reading. **Four rows
belong to this phase and all four are named here rather than the two that succeeded**:
1789 is the one-month probe below, 44,099 ms over 486,890 rows, and **1788 is the `0014`
key defect**, `PercentileEngine`, `failed`, run_date 2026-07-01, 73,323 ms, no rows. A
reader checking the log against a record that described that failure in prose alone would
find a failed run the record did not point at.

**C11 was measured over one month before the window was committed to**, at 23 dates in
43.9 seconds [`run_log` **1789**, 44,099 ms], which is 1.9 seconds a date and put the
window at roughly 46 minutes. That
...
UPDATE command cannot affect row a second time` [`run_log` **1788**, `failed`]: that date carries one `mid` name with an
...
truncated the table at 2016-01-04 and the ticker count is what it left.~~ **Corrected
2026-08-23, and the error is this section's rather than the store's.** 50,785 is the
admitted pool 3.1 measured, not a `price_daily` ticker count, so the sentence set a table
against a population it was never the same set as. The phase 3 record's own `price_daily`
ticker counts are larger than the pool rather than smaller, the nightly bulk feed writing
every US instrument and no sweep touching an ETF; they stay in that record and are not
restated here.

**What stands is the figure this section needs. `price_daily` holds 4,326 distinct
tickers**, of which **4,306** carry a bar inside the window, measured 2026-08-23. **This
is where the phase 3 record now points for the current ticker count.** It does not point
here for the size, which has not moved since the prune and whose home is the prune record
[item 51]. Every size in this section is measured against that pruned store and would
grow roughly in proportion to the evaluated pool on a restored one; the three §16 rows
say so.

**Two things the store has done since the prune, observed here and not acted on.**
`MTHI.US` carries 1,369 bars from 2010-07-27, so the table is no longer truncated at
2016-01-04 as the prune left it, and seven dates after the 2026-08-12 frontier carry
between 12 and 22 bars each. Both contradict phase 3's definition-of-done line 1, which
reads the table as beginning where the prune cut it and ending at the ingest frontier. Neither is this phase's and neither reaches
the record inspector, which reads one date at a time. Recorded because the pointers this
section is now the destination of would otherwise carry a reader to a paragraph that
...
|---|---|---|
| Names carrying all three of `article_count_z_own_90d`, `sentiment_delta_7v30` and `sentiment_7d_level` | 453 of 2,841 | ~~608 of 2,560~~ **562 of 2,560** |
| Names carrying `article_count_z_own_90d` alone | not read | **608 of 2,560** |
| Rows the median member has inside the 90-day baseline | 10 | ~~11~~ **12** |
| Rows `AAPL.US` has in 30 days against 90 | not read | **30 against 90** |

**Two of those figures are corrected at the sign-off and the first one is a label rather
than a query** [3.5 sign-off]. 608 is the article-count z-score column by itself, written
under a row naming all three, so the number and its label counted different things and
the comparison against phase 2 was between two populations. All three together is **562**
on that date.

**The three diverge after the floor, which is why one number cannot stand for them.**
`sentiment.min_baseline_days` gates all three alike, and past it
`article_count_z_own_90d` is null only where the count never moves, while
`sentiment_delta_7v30` and `sentiment_7d_level` both need a scored row inside the last
seven days. The 46 names between 608 and 562 are exactly that state: cleared the ninety
day floor, nothing scored in the last week.

**Which of the two phase 2's 453 counted cannot be established and is not guessed here.**
That store no longer exists, the backfill having rewritten `sentiment_derived_daily`
whole, and phase 2's own sentence attributes the figure to the baseline floor, which
gates all three and so distinguishes neither. What the row can carry is both readings
with their columns named, which is what it now does; a later reader comparing them knows
which is which.

The median is **12** over the 2,560 members by `percentile_disc`, unchanged by the window
edge being inclusive or exclusive and unchanged by `percentile_cont`, and 13 over the
names carrying any row at all. Not 11.
...
### The corrections the review's findings were directed into, 2026-08-23

Human-directed, in the session that ran the review. **Six of the nine exceptions above are
closed here and the other three are not**, being authored questions rather than
corrections: the `Universe.AsOf` copy, the authorship record for `ARCHITECTURE.html`, and
the three per-checkpoint checks the plan asked for and the record does not evidence.

#### The panel defect, corrected in the reader rather than in the store

**`RecordInspector.RejectionAsync` takes the in-force evaluation date as a lower bound**,
so the as-of pick spans both tables instead of being two independent picks that can both
answer. One evaluation writes to one of the two tables and the answer is whichever C01
reached last, which is what `SCHEMA.md` means by the two partitioning the evaluated
population.

**The bound is inclusive and that half is measured rather than reasoned.** A name leaving
the universe is written both rows on the one evaluation date, the departure into
`security_daily` and the criterion here, and **7,097 departure rows share an evaluation
date with their criterion**, so a strict bound would have hidden the only thing that says
why each of them left. That is the case that makes this a correction and not a blanket.

**Proved against the store as well as by test.** `ABNB.US` on 2024-06-05 reads its
membership row in force at 2024-06-02, active; the unbounded read returns
`insufficient_history` at 2021-12-05 and the bounded one returns nothing. Three tests,
registered in `FIXTURES.md`, and the double applies the bound the statement carries, so
the corrected reader passes where the uncorrected one fails rather than the other way
round.

**Nothing in the store changed and `Record.razor` was not touched.** The rejection block
is already conditional on a rejection being present, so with the reader corrected a member
renders no rejection and a departure still renders its criterion.

#### What was struck and what was clean-edited, and why they differ

`ARCHITECTURE.html` §16 took a clean edit with its prior wording in `CHANGELOG.md`, being
a document read to know the current state. Everything else here is struck in place,
`PROGRESS.md` being the record [D-73].

**The C01 timing row is in this file rather than in `ARCHITECTURE.html`**, which is worth
stating because the correction was directed as though it sat with the §16 sentences. It
takes the record rule accordingly. Correcting it turned up a second measurement the row
had already missed: `run_log` 1780 at 10.06 minutes, from the 3.16 sequence on 2026-08-22
and already sitting in the 121.30 table further down this file. So the clause said "not
re-run" while the same document recorded two later runs of the stage, and the review found
the newer of them rather than the first.

#### The null close, filed rather than fixed

**Item 64.** Recorded as unreachable with the measurement rather than as safe: the store
holds zero rows with a null close, read 2026-08-23, which is a property of the store on
one date and not of the code. The choice between rejecting on a new criterion, restoring
the null exclusion inside the `CASE` and leaving it to throw is authored, and `0015`'s
`CHECK` has no value for a name with no readable close.
...
## Phase 4, item 42 authored as D-130, and the phase signed off, 2026-08-24

**The last open exception the sign-off review returned is authored.** D-130 states that a
session is a date the exchange traded, read from the exchange calendar rather than inferred
from the store, and closes item 42 and the half item 41 shares with it. Authored on the
operator's direction. **No code moved in this pass** and no migration was written: the
decision is authored, the code change is filed, and nothing was run against the frozen
record [`RUNBOOK.md`].

### What D-130 decides, and what it rejects

**The reasoning behind taking a range's dates from `price_daily` holds for the walk and not
for the source.** A calendar walk would write a row of nulls on a closed day,
indistinguishable from a name with no history, which is the failure that reasoning was
arranged against; taking the dates from the store instead means a handful of stray bars
makes a closed day a session. Both are avoidable at once by reading the answer off the
exchange calendar.

**A bar-count threshold is rejected** on three grounds recorded in the decision: no number
in this corpus constrains where it would sit, it would need revisiting whenever the
provider's coverage changes, and it infers a fact `ExchangeCalendar` already holds as data.

**Item 41's option 3 is not this decision.** A calendar taken from one benchmark ticker's
series would have excluded exactly these dates and risked excluding real ones on any gap in
that one name. The exchange calendar depends on no ticker's coverage, which is what makes
the two different answers rather than the same one arrived at twice.

### The three things that close, and what stays

| | |
|---|---|
| Item 42 | **Closed by D-130**, and later than it asked for: it said the decision was needed before phase 4 read any of it, and phase 4 read all of it |
| Item 41's shared half | **Closed by D-130.** The loose index scan settled the access path at 6,328 ms and left the source unexamined |
| Item 57's closure note | **Amended.** It read as closing more than it did: its sentence covers a date with **zero** bars, which is what its predicate selected, and does not reach a closed day carrying a handful. The scope is now stated |
| The 57 frozen rows | **Stay.** 8 candidates and 49 shadows on 54 days the exchange was shut, and `attribution` is never rewritten [INVARIANT 4] |

### The two obligations filed, and why they are two

**Phase 8 owes an exclusion rather than a correction.** `attribution`'s nine return columns
are counted in trading days, so a forward window spanning a closed day is read one session
short and errors on nothing, which is item 57's own statement of why it mattered. The
obligation has two halves that are deliberately not one: **the rule**, that C21 counts
forward sessions off the exchange calendar; and **the eight rows**, named by date rather
than described, so they are excluded by identity rather than by a rule that has to be got
right twice. They are 2022-07-04, 2024-01-15, 2024-02-19, 2024-11-28, 2025-01-01,
2025-01-09, 2025-02-17 and 2025-11-27. The 49 shadow rows on the same dates take the same
exclusion, and C22 and C24 read the same table.

**The code change is filed against whichever phase next touches the calendar**, naming
`TradingCalendar.SessionsAsync`. It is one statement in one place and its reach is not: C08,
C09, C10, C11 and C35 all take their dates through it, so it is rebuild-forcing and bundles
under `SCREEN_LIFECYCLE.md` §6.7 rather than being taken alone. A build session does not
widen its own scope to take it [`CLAUDE.md` §3].

**The two authored size figures ride with the `3 → 3 sign-off` obligation**, where the rest
of §16's measured-versus-estimated question already sits: `screen_score_daily` at "~280 MB
per registered screen" against 4.13's measured 305, and the database-size row measured
before this phase added three tables.

### Both measurements are in the decision rather than only in this file

**Deliberately, so the decision is not re-opened on intuition.** D-130 carries the floor
figures, under 0.1 percent of the floor and under a name a date, and it carries S1's D-1 at
0.8784 against 0.9344 and D-5 at 0.7683 against 0.8093 with the direction named: the
recorded persistence figures understate rather than inflate, which is the direction that
gets believed without checking.

### Sign-off

**Phase 4 is signed off 2026-08-24 on the operator's direction.** Step 1 is met with
`ci.ps1` green at the branch head. Step 2 ran in a session that had not committed here,
returned six exceptions and six smaller items, and all of them are now closed, filed or
authored. The phase status row is restated.

**Nothing about the frozen record changed in this pass and nothing could.** 74,767
...
## Phase 5, checkpoint 5.2: all schema, before any component and before a row

Migration `0022` against `headline` and `news_digest`, both holding zero rows, with
`SCHEMA.md` and `FIXTURES.md` in the same checkpoint. **`ci.ps1` green at `a2db04e`**:
guards exit 0 with 5 checks over 186 files, build 0 warnings and 0 errors, migrate from an
empty server applying **22 migrations**, migrate again with nothing to apply, and **735
passed, 0 failed**. Thirteen tests added against the 722 baseline.

### What landed

**`headline.content`, nullable** [D-131]. The column the digest actually reads, which the
table could not hold: `ARCHITECTURE.html` §07 prices the whole local-enrichment step
against articles of five to eight hundred tokens and `headline` carried only a title.
Nullable because 5.1 found the endpoint may send an article without a body, and null there
means the provider sent none rather than that it was empty. **`text NOT NULL DEFAULT ''`
is the shape that suggests itself and it makes absent and empty the same value**, at
exactly the grain where D-134's four outcomes need them separable, so a test asserts the
three states apart.

An index on `(ticker, published_at DESC)`. The `0001` index is on `(ticker, date)`, which
is the run's date rather than the article's, so the column D-133 orders on had none.

**`news_digest.provider` closed by a CHECK** to `local` and `haiku`, with `DigestProvider`
in `Core` holding the same list and a test reading the constraint out of the catalogue to
assert the two agree. This is `0018`'s instrument for the gate reasons and `0021`'s for the
alert types, applied to the column that says which model produced the evidence [D-29,
D-134].

**`news_digest.model_name` required non-blank.** NOT NULL was already there and an empty
string satisfies it.

**`attribution.digest_provider` untouched and null for ever** [D-135], with the check built
against a fabricated NewsDigester claiming `attribution.Update`, because neither that
component nor ForwardReturnFiller exists and the assertion over the real registry passes
over a set of one.

### The first form of the blank check was wrong, and the fixture found it

`btrim(model_name) <> ''` was written, and the theory beside it asserts an empty string, a
single space and a single tab. **The tab passed.** `btrim` with one argument trims spaces
alone, so a model name of one tab satisfied a constraint whose whole purpose is that a row
cannot carry a provider and no model.

It is now `model_name ~ '[^[:space:]]'`. **A regex rather than an escaped trim set, and
that is not a style preference**: `E' \t\n\r'` written into a `.sql` file is either escape
sequences or the characters themselves depending on how it was written, and the first
attempt put a literal newline and a carriage return inside a migration whose text is
hashed and recorded. The regex says the same thing in printable characters.

**Recorded because it is the phase's first defect and it was found by running rather than
by reading.** A constraint holding for two of three blank cases reads exactly like one
holding for three.

### What 5.2 deliberately did not do

**`headline.source` gains nothing and is written null** [5.1]. The `news` payload carries
`content`, `date`, `link`, `sentiment`, `symbols`, `tags` and `title`, and nothing maps to
`source`. The host of `link` is derivable and deriving it is a choice rather than a read.
This is `slot_filled` [D-124] and `attribution.digest_provider` [D-135] a third time and
the disposition is the same one.

**`sentiment` and `tags` gain no columns.** Both are carried by the payload and neither is
asked for by any document.

---

## Phase 5 stops at 5.3, and what it stops on

**Three things block the next checkpoint and all three need a decision or a credential.
None is a build session's to close** [`CLAUDE.md` §3, §13].

### Blocker one: `digest.max_articles` cannot be seeded

**5.1's stopping rule fired and this is it working rather than failing.** D-133 puts the
cap at 3, derived from §07's own cost figures rather than chosen: the rotation at about
$1.30 a year and a full secondary year at roughly $18 leave about 1,800 input tokens per
candidate, which at §07's five to eight hundred tokens an article is three.

**The measurement says the median article is 1,248 real tokens.** Three articles is 3,745
tokens against the derived 1,800, and the tail is worse: p95 is 4,777 tokens for one
article and the longest body in the window is an earnings-call transcript at 14,099.

So the cap is a decision rather than a seeded value, which is what the stopping rule's
above-800 branch says. **Nothing was seeded and no cap was chosen.**

**Three options, none taken here.** Keep three articles and accept that a full secondary
year costs about $37 rather than $18 and the rotation about $2.70 rather than $1.30, both
small in absolute terms. Cap by tokens rather than by article count, which is the option
the tail argues for and which no document currently describes. Or keep three and truncate,
which D-137 forbids for a digest and would need saying explicitly for an input.

**Config is append-only and versioned, so seeding a cap now and changing it later is a
config version and a history that must be segmented rather than pooled** [`CLAUDE.md` §8,
§12]. That is why this waits rather than being seeded provisionally.

### Blocker two: the local link returns no digest at all

**Measured at 5.1 and it is not a parameter to tune.** `qwen3.6:latest` is a reasoning
model. At `max_tokens` 150 it spends all 150 on a `reasoning` field and returns zero
characters of content with `finish_reason: length`. At `max_tokens` 2000 the content is
**still** empty. `chat_template_kwargs.enable_thinking = false` is accepted and ignored.
`reasoning_effort: "none"` produces a digest in 74 completion tokens.

**Under D-137 as authored, this makes every night run on the paid link.** An empty response
is malformed, so the chain retries once, gets another empty response, marks the local link
unhealthy for the run, and digests all 32 candidates on the secondary. With the local
server up and reporting healthy.

**What needs authoring is not the parameter but two things the corpus does not say.**
First, that the local link requests no reasoning, and where that setting lives: a config
key, a `local_model_config` column, or the client. §07 names LM Studio and the machine runs
Ollama, and `reasoning_effort` is an OpenAI-compatible parameter rather than a universal
one, so a setting that works here may not work on the server §07 names. Second, that
`digest.max_tokens` is two quantities: the digest's length, which is what §07 and
`CONFIG_REFERENCE.md` mean by 150, and the completion's length, which is what the API
parameter caps. On a non-reasoning model they are the same number.

**It blocks 5.5 and 5.7.** Building the local client and the chain against an unauthored
answer would be building the wrong thing, and the failure would look like a working chain.

### Blocker three: the Anthropic key is a placeholder

Eighteen characters and it does not begin `sk-ant`. **Blocks 5.6 and 5.14**, and blocked
5.1's rows 5 and 6, which would have measured the `usage` block D-140's cost row is shaped
against.

### What is not blocked, and why it was not built anyway

**5.4, C29 HeadlineIngestor, is blocked only by ordering.** Its decisions D-131 and D-132
are authored, its schema landed at 5.2, and it does not read `digest.max_articles`: the cap
is the digester's, applied at selection, and C29 stores the window. What it does read is
`digest.lookback_days`, which 5.3 seeds and 5.3 is blocked.

**Seeding the nine decided keys and leaving the tenth is available and is not a build
session's call.** `ConfigResolutionTests` already covers a key seeded by a later phase,
backdated as the seeder backdates, with prior dates asserted unmoved, so a tenth key
arriving later is an ordinary operation rather than a history split. It would make 5.3 a
partial checkpoint, which is re-drawing the checkpoint and therefore authored scope
[`BUILD_PLAN.md`, a commit spanning two checkpoints means the checkpoints were drawn
wrong].
...
| Checkpoint | State |
|---|---|
| 5.1 | **Done.** Six measurements, two of them stopping the build. Transcript committed |
| 5.2 | **Done.** `0022`, `SCHEMA.md`, `FIXTURES.md`, 735 tests, `ci.ps1` green at `a2db04e` |
| 5.3 | **Blocked** on blocker one |
| 5.4 | Blocked by 5.3's ordering only |
| 5.5, 5.7 | **Blocked** on blocker two |
...
## Phase 5, checkpoint 5.11, the gate

**The halt already existed in C33 and what 5.11 adds is the observable** [INVARIANT 15,
D-139]. A stage that throws is not evidence that a night stops: what had to be asserted is
that nothing after it runs, that the table is untouched, and that the failure names the
invariant rather than reading as an ordinary error an operator restarts past.

**Four assertions, and the fourth is the one the live halt taught.**

| What | Why it is separate |
|---|---|
| C33 fails, naming INVARIANT 15, D-139 and both links with their own reasons | A halt that reads as a transport error is one somebody retries past. The per-link reason is D-144's distinction: a machine that is down and a model that answers with nothing call for opposite responses |
| `news_digest` holds no row for the date | A partial digest set is the state nothing downstream can tell from a real one [`CLAUDE.md` §6] |
| `NightlyRun` returns not completed and the stage after C33 never executes | "Nothing after it ran" is a property of the sequence rather than of the component, so it is asserted through `NightlyRun` with the order passed explicitly, C29 and C33 not being in `EveningOrder` until 5.12 |
| A halted run leaves an earlier run's rows exactly as they were | The delete and the insert both sit after the probe. The natural shape of a re-run is delete-then-write, and a delete that ran before the probe would empty a night on its way to failing |

**The fourth was observed before it was written.** On 2026-08-25 a digest re-run met a local
model that had gone non-resident, the probe exceeded `digest.health_timeout_ms`, the
stand-in secondary was unhealthy, and the run stopped with the night's 32 rows intact. The
test is that night made repeatable.

### D-139's ordering consequence, asserted and proved against a counter-example

**C28 must precede C29 and C33**, because §04's 19:00 for the concentration monitor is a
clock time rather than a dependency: it reads `candidate_set`, `position` and
`security_daily` and nothing the digest step writes, and a night that halts is not a night
its guarantees stopped mattering.

**Until 5.12 puts the two stages into `EveningOrder` that assertion holds by their absence**,
which would make it a check that tested nothing. So the rule is proved against two
fabricated orders that violate it as well as against the real one. When 5.12 appends C29 and
C33 the same test becomes load-bearing without being touched.

### Both forward fixtures are registered, and one of them moved position

`FIXTURES.md`'s two phase-5 forward items are now rows: the night the primary is unavailable
and the chain falls through, and the night no provider is healthy and the run halts. **The
first is registered here rather than at 5.7 deliberately**: 5.7 proved a call falling
through and the forward list names a night, which is the digester's run over a candidate set
with the reason reaching the run log.

### What 5.11 does not evidence

**The done-when line as `BUILD_PLAN.md` states it is still not evaluable and is still
re-asked at phase 7** [D-139]. "Stopping both halts the run before any researcher call and
produces no orders" names two things that do not exist. What is proved here is the narrower
observable in full. **The evidenced half is the halt, the empty table and the stopped
sequence; the half that waits is that no researcher call and no order followed, which needs
phases 6 and 7 to exist before it can be false.**
...
## Phase 5, the same night re-digested after 5.10, so the rotation and its fall-through are on the record

**Run once 5.10 existed, over the same 275 `headline` rows and the same 32 candidates**, with
the stand-in at the chain's second position changed in one respect: it answers with nothing
rather than throwing. An unbuilt link is not a broken chain, and what the chain does with a
link that answers nothing is D-137's refuse, retry once, fall through.

| Step | Count |
|---|---|
| `news_digest` rows | **32** |
| Articles sent | **81**, the same 81 the previous run sent |
| `was_rotation` true | **2**, `COUR.US` and `PAYC.US` |
| `provider` on those two rows | **`local`**, both, the second position having failed |
| prose, `NO MATERIAL NEWS`, null | **14, 15, 3** |
| Wall time | 123 seconds |

**The two rotated rows are the dead-secondary state recorded rather than suppressed.** The
rotation selected them from the date and the tickers before anything was digested, the
second position could not answer, and they fell through to the first link and stayed marked.
That is exactly 5.10's reading of D-138 seen against a real model, and the run log carries
both halves of it:

> ... 2 rotated to haiku. Passed over: haiku at position 2 failed twice and is unhealthy for
> the rest of this run: the link answered with no content

**Two calls to the dead link and not two per rotated candidate**, which is the count D-137
states and which the suite asserts against stubs. Here it is the same count against a run of
32.

### A re-run of C33 over one night is not byte-identical, and nothing says it should be

**The selection reproduced exactly and the output did not.** Both runs sent 81 articles, so
D-143's ordering and cap are deterministic over the same store, which is what INVARIANT 6
needs of the input. The outcomes moved by one: 13 prose and 16 `NO MATERIAL NEWS` on the
first run, 14 and 15 on this one, over the same evidence.

**That difference is the model and not the pipeline**, and it is worth stating because C29
carries the opposite guarantee: a re-run of the headline ingest over one date is asserted
byte-identical as a set comparison in both directions [5.4]. The digest step has no such
assertion and cannot have one while a model is in the middle of it. **What must stay
identical is what the model is sent**, which is the instruction hash and the article
selection, and both did.

**It also sizes something D-27 will meet.** The rotation compares two models on the same
...
### The live half, run by the operator on 2026-08-27 at ~~04:03~~ **00:03** ET

**The hour was the UTC one, corrected at the sign-off review.** The `run_log` row for this
run reads `2026-08-27 00:03:05.42266-04` and the commit recording it, `49e235e`, is dated
`00:07:33 -0400`, so 04:03 is that instant in UTC and the label was ET. All market
semantics here are US Eastern [`CLAUDE.md` section 6]. Nothing downstream reads the hour.
...
**Every owed line is now met or deferred, as of 2026-08-27 ~~04:03~~ 00:03 ET.** Two are met, 5.12's
