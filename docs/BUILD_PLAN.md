...
gate. **C29 HeadlineIngestor as well, and it was missing from the line above rather
than deferred** [amended 2026-08-24]: `headline` had no writer in any phase in this
document, C33 reads `headline`, and a digest step over an empty table is D-134's
no-article outcome for every candidate on every night. **C26 CostLedger's digest half
as well** [D-140], because D-27's rotation makes this the first phase that spends
money and phase 6's carried obligation binds one phase early.
...
narrower: the run halts at C33, `NightlyRun` returns not completed, no stage after
C33 executes, and no `news_digest` row is written. **The line as written is re-asked
at phase 7** rather than marked met here.

**Two lines are added, both because the phase cannot be read without them.** A re-run
of C29 over one date is byte-identical, asserted as a set comparison in both
directions, which is D-132's grain rule made observable. And one candidate's digest
can be opened beside the articles it was built from without writing a query, which is
5.9 and is this phase's openable deliverable rather than a nicety.

### Checkpoints

| # | Scope |
|---|---|
| 5.1 | The sweep, before any component and before any schema. Six measurements from a scratch app outside the repository, transcript to `docs/evidence/phase-5/`, token never printed. **The article-length distribution recorded against §07's five-to-eight-hundred-token estimate**, which is what makes `digest.max_articles` measured rather than derived. **It carries a stopping rule**: a `content` field that is a teaser rather than a body halts the build before 5.2, and a median body outside 300 to 800 tokens is reported and waited on rather than seeded. Nothing is committed under `src/` |
| 5.2 | All schema, before any component and before a single row. Migration `0022`: `headline.content`, `news_digest`'s non-empty `model_name` CHECK and its closed `provider` vocabulary, and `attribution.digest_provider`'s disposition stated. `SCHEMA.md` in this same checkpoint, both parity directions green, `ExpectedMonetary` asserted unchanged. Blocked on D-131, D-132, D-134 and D-135 |
| 5.3 | Config: the `digest.*` namespace seeded including D-133's two keys and D-140's model id, `s5.no_digest_disqualifier_min_articles_90d` seeded at last, and `local_model_config`'s two rows through `ConfigSeeder`. `ConfigSeeder.Keys.Count` moves from 95 deliberately and the test moves with it. Blocked on D-133, D-136 and D-140 |
| 5.4 | C29 HeadlineIngestor. Reads `candidate_set` and the news endpoint through `EodhdClient`, writes `headline` under D-132's run scope. **A night with no candidates writes no rows and that is correct**, asserted through `ZeroRowsExpected` rather than tripping the zero-row halt. Blocked on D-131 and D-132 |
...
| 5.6 | The secondary link, through the Anthropic SDK at `claude-haiku-4-5`. Same interface, same instruction text read from `prompts/digest-instruction.md`. **It reads no store and is therefore not a registry component.** Blocked on D-136 and D-140 |
...
| 5.8 | C33 NewsDigester and the write. D-133's selection, D-134's four outcomes, provider and model name on every row. **Its declared read set names `candidate_set` and `headline` and nothing about a score**, which is INVARIANT 7 made structural rather than remembered. Blocked on D-133 and D-134 |
| 5.9 | **C36 gains a fifth panel, the digest panel**, and it is this phase's openable checkpoint. For one ticker on one date: the articles selected with their dates and bodies, the pool they came from, the instruction file's version, the digest, the provider and model that produced it, and which of D-134's four outcomes the row is. It reads `headline` and `news_digest`, declares both, and computes nothing [D-109]. **It is not early and cannot be**, the second table holding nothing until 5.8 writes it, so the earliest honest position is immediately after the write. Blocked on D-131, D-133 and D-134 |
| 5.10 | The rotation. D-138's hash, `was_rotation` on exactly `digest.rotation_count` rows, present on a night the primary is healthy and on a night it is not. **Two adjacent dates over one candidate set produce different pairs.** Blocked on D-138 |
...
| 5.12 | The evening order wired, C29 at 18:32 and C33 at 18:33, and one real night run end to end. **The observable is a chain of counts.** `RUNBOOK.md`'s cycle table amended with the order. Blocked on D-139 |
...
**Invariants at risk:** 7, 10, 15, and **6 and 11 are added** [2026-08-24]. **6**
because the instruction text reaching the model must be byte-identical across every
call within a night, which is the property the prefix has one phase later, and a
locale-formatted date or an unordered article list inside the prompt breaks it the
same way. **11** because a health check is the most natural place in this system to
reach for a clock, and `last_health_check` is a column shaped exactly like an
invitation to.

**What this phase does not do.** No backfill of headlines or digests, on three
grounds set out in the phase plan: a backfilled digest is model output produced today
and is not point-in-time, nothing downstream reads it, and 34,932 historical
candidates at three articles each is 104,796 news calls. No dossier, no researcher,
no proposal, no order. No judgement of digest quality, the rejection surface being
transport and length only [D-137, INVARIANT 7]. And no answer to D-53, which needs a
quarter of the rotation's paired sample.

**The detail is in `prompts/BuildPlans/phase-5-digest-chain.md`**, which carries the
derivation of D-133's three articles from §07's own cost figures, 5.1's stopping rule
in full, and the seven findings this phase reports.
...
| 1 | 5 | ~~`headline` is an event record, same distinction~~ **Closed by D-132, 2026-08-24.** Idempotence is by run scope rather than by row identity: C29 deletes the ticker and date it is about to write and reinserts, both operations its own, which is C14's shape one table over. Two articles identical on every attribute but the body stay two rows |
| 5 | 9 | `local_model_config.last_health_check` and `last_loaded_model` have no writer that can run. The declared writer is the UI [D-51], the UI is phase 9, and C32's Writes cell in `ARCHITECTURE.html` §03 says nothing, via C27. D-136 leaves both null and routes health through the run log, which is consistent with the catalogue and with D-124's treatment of `slot_filled`. Phase 9 decides whether the UI fills them; until it does, §20's U6 line "last successful digest run" reads `run_log` rather than this table |
...
| 4 | 8 | **S3's composition, which its measured tilt puts in question and which Q.10 deliberately did not touch.** §05's cell read "Tilts small" and S3 ranks **57.8 percent** large; the cell is corrected at Q.10 and the screen is not. **The evidence is that the tilt has two separable stages and only the second is about ranking.** Its three inputs compute for **1,067 of 2,819** names, and that covered population is already **55.0 percent** large against a universe at 31.3, so the screen is large-tilted before it ranks anything; the ranking then carries it from 55.0 to 57.8, which is **2.8 points**. It is the only one of the five that ranks larger than its own scored population. **The first stage read 47.0 until the phase 4 sign-off review**, which found that figure to be S3's coverage over 2026 to date, the lowest of the six years, stated under a whole-range heading; over the range the evidence is stated on it is 55.0. That makes the question sharper rather than softer: almost the whole tilt is set by which names have coverage at all, and recomposing the ranking cannot move it. Three answers are open and none is obviously right: recompose the screen, rebase its ranking against its covered population rather than the universe, or accept it as the sentiment reading it is and stop calling it small. Owed to phase 8 because C22 ScreenTuner and the paired comparison are what would read it: a tuner moving slots on peer-relative performance is allocating to a screen whose population differs from the other four's, and D-42's peer-relative column is the thing that hides it. Beside that, S3's persistence figures are the closest of the five to chance, D-5 at 2.6 times chance and D-21 at 1.2 [4.13], so the two questions land on the same screen and are answered together or not at all |
| 4 | 8 | **S5's composition, filed with S3's above and for the same reason: the correction at Q.11 is to the document and the screen is unchanged.** §05's Typical fill cell read "0 to 5, median 3" and S5 filled **68 of 35,108** live seats over five years, 0.19 percent. **The evidence is that the shortfall has two stages and only one of them is separable from the record.** The gate admits a mean of **2.4** names a date, already below the estimate's median before anything is ranked, and the 98th-percentile floor over a trailing distribution that small then ranks one name on **70 of 1,457** sessions, 68 of which reach a seat. So the gate is the binding stage. **Why the gate is that tight is not separable**: a failing name carries a null score and `IS NOT TRUE` collapses failed and unevaluable into it, so `screens.S5.quality_quintile_min` at 80, `screens.S5.technical_quintile_max` at 20 and the two composites' composition are one bit per name-date and the store distinguishes none of them. The two news conditions failing open are excluded on the gate's shape rather than on the record, that branch being a disjunction that can only widen what is admitted. Separating the rest needs the gate re-evaluated condition by condition, which is a measurement and not a query [`CLAUDE.md` §3]. **A screen contributing 0.19 percent of seats is a slot allocation question before it is a composition one**, which is why this lands with the tuner: §8.1's proportion and D-43's floor of four are what decide whether a screen this quiet keeps slots at all |
| 4 | 8 | **C21 ForwardReturnFiller skips a non-session date, and the eight rows already standing on one are excluded by name** [D-130, phase 4 sign-off]. `attribution`'s nine return columns are counted in trading days, so a forward window spanning a day the exchange was shut is read one session short, and it errors on nothing [item 57's own words]. Two things are owed and they are different. **First, the rule**: C21 counts forward sessions off the exchange calendar and not off the dates `price_daily` holds, which is D-130 applied at the point the columns are filled. **Second, the eight rows**, which are candidates frozen on days the exchange was shut and cannot be rewritten [INVARIANT 4, `RUNBOOK.md`]. They are **2022-07-04, 2024-01-15, 2024-02-19, 2024-11-28, 2025-01-01, 2025-01-09, 2025-02-17 and 2025-11-27**, one candidate each, and they are named here rather than described so they are excluded by identity rather than by a rule that has to be got right twice. **49 shadow rows stand on the same dates** and the same exclusion reaches them; the whole set is the 57 `attribution` rows on the 54 dates. **A correction is not available and is not what is asked for.** These rows carry scores frozen as they stood, and the exclusion belongs in whatever reads them: C21, C22 ScreenTuner and C24 CalibrationReporter |
| 4 | whichever phase next touches the calendar | **`TradingCalendar.SessionsAsync` takes a range's dates from the exchange calendar rather than from `price_daily`** [D-130]. The component is named because the change is one statement in one place and its reach is not: C08, C09, C10, C11 and C35 all take their dates through it, so this is rebuild-forcing and bundles under `SCREEN_LIFECYCLE.md` §6.7 rather than being taken alone. `ExchangeCalendar` already holds a working-day set and a dated holiday set and is what the answer is read from. **The evidence is measured and is in `PROGRESS.md` rather than restated here**: 1,584 distinct dates against roughly 1,409 real sessions with 162 below 1,000 tickers [item 42]; and, taken at phase 4's sign-off, floors moving by under 0.1 percent of themselves and under a name a date when the 54 leave a 250-date lookback, against S1's D-1 rising 0.8784 to 0.9344 and D-5 0.7683 to 0.8093, which is the one figure the session list was carrying and it carries it in the flattering direction |
...
| 5 | 6 | **`digest.chain` is versioned and `local_model_config` is not, and `DigestChain.BuildAsync` compares a count across them.** So the name list resolves as of the simulated date and the enabled set resolves as of now, and the comparison has no consistent as-of. It already bit once, on 2026-08-26: a `digest.chain` edit cannot be in force on the first night after it is made, because the guard blesses the previous session and every stage after it runs on that date. Nothing else in this corpus pairs a versioned key with an unversioned table, so the exposure is this one key. Two shapes close it and both are authored: `local_model_config` gains a version, or `digest.chain` becomes the sole declaration of membership and `BuildAsync` stops reading `enabled`. **Decide before a past date is expected to run through the composed pipeline**, which is when 5.6's link is live |
| 5 | 9 | **Clearing `local_model_config.request_options.model` does not fail the run and changes which model writes the night's evidence.** `LoadedModelAsync` takes the ordinally first id from `/v1/models`; on the configured endpoint that is `prism-ml/bonsai-27b` and the configured model is `qwen/qwen3.5-9b`, so the row's key is what keeps the right one answering [D-144]. Without it a model nobody chose answers, the probe reports it as the model that answered, and only `news_digest.model_name` records what happened [D-29]. It has no test, the pick being a property of a live endpoint's listing rather than of the code. The remedy offered and not taken is a `model` column on `local_model_config`, which needs a migration, `SCHEMA.md`, an amendment to 5.5's tested behaviour and a decision number. **Phase 9 owns `local_model_config`'s writer** [D-51] and is where the column would land |
| 5 | 9 | **`digest.readiness_check_et` has no consumer and is not retired.** 5.13 was reshaped from a 15:30 clock check to a precondition ahead of C33, so nothing reads the key; `CONFIG_REFERENCE.md`'s Consumer cell says so rather than naming one that will not arrive. Config is append-only, so a retirement is a version with its own reasoning rather than a deletion. Decide it with the run health screen, which is the last thing that might have wanted a clock |
