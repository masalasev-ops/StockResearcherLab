# Shadow screens and screen retirement — the design brief, as issued

    Target:    Claude Code
    Issued:    2026-08-11
    Status:    SPENT
    Archived:  after the deliverable, per `CLAUDE.md` §3 as it stands.

**Verbatim, with one omission the brief itself directs.** Nothing below is summarised,
reordered or corrected. The brief's own §"What was settled while the brief was being
answered" is not here, because that section is not the brief: it recorded the positions
settled across three rounds of amendment while the work was being done, and the brief
says it is "the part that does not travel to the archive". Each of those positions is
carried into `docs/SCREEN_LIFECYCLE.md` with its reasoning, and the section itself
remains at `prompts/BuildPlans/design-screen-lifecycle.md`.

**The deliverable is `docs/SCREEN_LIFECYCLE.md`.**
