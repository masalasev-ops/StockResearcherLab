...
| # | Item | Trigger |
|---|---|---|
| 62 | **Correcting a range end to the ingest frontier costs C03 and C05 their resume position, and D-99 named this as the thing to split before it happened.** Measured 2026-08-22 after item 60's guard landed. Three of the five sweeps key their remaining set on the range **start** and fall through unchanged: `PriceIngestor.cs:257`, `EventsIngestor.cs:278` and `SentimentIngestor.cs:243` all read `last_attempted_date = rangeStart`, and all three carry 2021-01-04 for their whole pool, 50,827, 19,726 and 19,726 rows. **C03 and C05 key on the range end**, `FundamentalsIngestor.cs:85` and `FlowIngestor.cs:161` resolving `context.ForDateAsync(context.To)` and `:202` and `:344` reading `last_attempted_date = asOf`, and every one of their rows carries **2026-08-13**, the end item 60 proved wrong. So the corrected end of 2026-08-12 matches nothing and both sweeps re-dispatch whole: **20,068 tickers at `backfill.weight_fundamentals` of 10 is 200,680 units**, and 2,864 members at the 83.5 units a member 3.9 day four measured is about **239,144**, **roughly 440,000 in all, about 4.9 provider days** against the 90,000 usable after `backfill.unit_reserve`. The sequence would halt inside C03 and never reach the compute half, which is what a sequence run is for. **This is not a defect in item 60's guard and the guard is what exposed it**: the range end was wrong before, and the sweeps' resume position was keyed to the wrong value being repeated. **D-99 anticipated it exactly**: "One column serves two purposes on C03, the rotation's freshness ordering and the attempt marker... **This is the thing to split if it ever bites, and it is not split now.**" It has bitten by a route D-99 did not name, a range end moving backward rather than a sweep end the rotation should not treat as fresh. The asymmetry itself is deliberate and documented, C03's nightly and sweep calls being the same call where C02's differ in depth, so what is wrong is the shared column rather than the choice of stamp. `Program.cs`'s own help text already warns an operator to pass both dates for this reason | Before the sequence driver 3.16 is invoked for the first time, which is the one thing standing between the compute half and a full-sequence run. Two routes and both are authored: split the column so the sweep marker stops sharing `last_attempted_date` with the rotation's freshness ordering, which is D-99's own named remedy and a migration; or accept that a moved range end re-buys C03 and C05. Item 44 is the same column read from the nightly side and the two want deciding together |
