...
| 5.13 | The readiness check. `digest.readiness_check_et` at 15:30, run as a Worker command because there is no UI until phase 9, reporting which link would answer and what model is loaded. **It writes nothing and decides nothing**, which is what keeps it from becoming a second gate |
