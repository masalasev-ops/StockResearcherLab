...
|---|---|---|---|
| 1 | **3.16, the sequence driver** | The compute half ran through the same `BackfillRun` the sequence uses, on the same range, so the work done and the `run_log` rows written are the sequence's own. Its ordering is covered in CI by `BackfillSequenceTests`, cut-out-source fixture included | **`BackfillSequence.ExecuteAsync` has never been invoked against the real store.** Its own ordering and fall-through did not run, because at any range end other than 2026-08-13 C03 and C05 re-dispatch whole for about 440,000 units and the sequence would halt inside C03 without reaching the compute half. That is item 44 |
...
| 3 | **The timing line** | **170.35 minutes, measured and NOT met** against "minutes rather than hours". The bound is not moved and nothing was tuned toward it. Verdict recorded: nothing depends on the rebuild being fast, so it closes as a measured finding rather than an open item | It is a **closed finding, not a met line**. The done-when as written is not satisfied and the figure is what it is. The four separate causes of its move from 203.55 are tabulated in the verdict section, because a total that moved for four reasons reads wrongly as one |
| 4 | **Items 44 and 62** | The mechanism is fully recorded from both sides, with line numbers, row counts and the unit cost. Item 62 is merged into 44 | **Open, and deliberately not authored.** One `last_attempted_date` column serves both the sweep's done-marker and the rotation's freshness ordering; D-99 named the split and deferred it, and it has now bitten. Three triggers are named on the item. The remedy is a migration and an authored decision, which is a reviewer's call rather than a build session's |
